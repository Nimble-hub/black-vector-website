...
function friendlyError(value: unknown) {
  ...
  if (value === "FORBIDDEN") return "You do not have permission to change that message.";
  if (value === "RATE_LIMITED") return "Stand by before sending another message.";
  return typeof value === "string" ? value : "The message could not be changed.";
}
...
async function forwardMutation(response: Response) {
  const payload = await response.json() as { error?: string; message?: unknown; deletedId?: string };
  if (!response.ok) return Response.json({ error: friendlyError(payload.error) }, { status: response.status });
...
export async function POST(
  ...
  if (!identity) return Response.json({ error: "Sign in to transmit." }, { status: 401 });
  ...
  const response = await env.CHAT_ROOMS.getByName(channel).fetch("https://chat-room/publish", {
    ...
    body: JSON.stringify({
      ...
      displayName: identity.session.user.name,
...
export async function PATCH(
  ...
  if (!isChatChannel(channel)) return Response.json({ error: "Unknown channel." }, { status: 404 });
  const identity = await actor();
  if (!identity) return Response.json({ error: "Sign in to edit a message." }, { status: 401 });
  const parsed = mutationInput.required({ content: true }).safeParse(await request.json().catch(() => null));
  if (!parsed.success) return Response.json({ error: "Message must be 1–500 characters." }, { status: 400 });

  const response = await env.CHAT_ROOMS.getByName(channel).fetch("https://chat-room/message", {
    method: "PATCH",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
      ...parsed.data,
...
export async function DELETE(
  ...
  if (!isChatChannel(channel)) return Response.json({ error: "Unknown channel." }, { status: 404 });
  const identity = await actor();
  if (!identity) return Response.json({ error: "Sign in to delete a message." }, { status: 401 });
  const parsed = mutationInput.pick({ id: true }).safeParse(await request.json().catch(() => null));
